...
class ContenutiAllegatiMigrator extends MigratorAbstract
{
    ...
    public function migrate()
    {
        ...
        $attachmentRecords = $this->getRedbeanHelper()->getRecord("SELECT contenuti_allegati.id, nome FROM contenuti_allegati, mimetype WHERE (id_mime = mimetype.id) ");
...
(name, size, state, insert_date, mime_id, user_id) (SELECT nome, size, 'active', NOW(), id_mime, 1 FROM contenuti_allegati WHERE id = '".$attachment['id']."' ) ");
...
(title, description, attachment_id) VALUES ('".$newAttachmentFilename."', '', '".$lastID[0]['last_insert_id']."') ");
            ...
            $this->getRedbeanHelper()->executeQuery("INSERT INTO zfcms_attachments_relations (attachment_id, reference_id, module_id) VALUES ('".$lastID[0]['last_insert_id']."', ".$attachment['id'].", '2' ) ");
